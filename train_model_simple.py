"""
Simplified training script for sentiment analysis.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('training_simple.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class SimpleSentimentAnalysisPipeline:
    """Simplified sentiment analysis pipeline without Word2Vec."""
    ...
    def _get_default_config(self) -> dict:
        ...
        return {
            'dataset': 'imdb',  # Use IMDB to avoid download issues
            'sample_size': 100,
            'test_size': 0.2,
            'val_size': 0.1,
            'random_state': 42,
            'feature_extractors': {
                'bow': {'max_features': 1000},
    ...
    def extract_features(self, X_train, X_val, X_test):
        ...
        # Initialize feature extractors (only BoW and TF-IDF)
        self.feature_extractors = {
    ...
    def evaluate_models(self, y_val, y_test, features):
        ...
        self.evaluator.save_results('results/evaluation_results_simple.json')
    ...
    def create_visualizations(self, df, evaluation_results):
        ...
        self.visualizer.plot_sentiment_distribution(
            df, 
            save_path='plots/sentiment_distribution_simple.png'
        )
        
        # Word clouds
        self.visualizer.plot_sentiment_wordclouds(
            df, 
            save_path='plots/sentiment_wordclouds_simple.png'
        )
        ...
        self.visualizer.plot_model_comparison(
            model_comparison,
            save_path='plots/model_comparison_simple.png'
        )
        ...
        for model_key, result in evaluation_results.items():
            self.evaluator.plot_confusion_matrix(
                ...
                model_key,
                save_path=f'plots/confusion_matrix_{model_key}_simple.png'
            )
    ...
    def generate_report(self, evaluation_results, df):
        ...
        # Save report
        with open('results/training_report_simple.json', 'w') as f:
            json.dump(report, f, indent=2)
        
        # Print summary
        print("\n" + "="*50)
        print("SIMPLE TRAINING SUMMARY")
        print("="*50)
    ...
    def run_pipeline(self):
        ...
        logger.info("Starting simplified sentiment analysis pipeline...")
...
def main():
    """Main function to run the simplified training pipeline."""
    # Configuration
    config = {
        'dataset': 'imdb',
        'sample_size': 100,  # Small sample for quick training
        'test_size': 0.2,
        'val_size': 0.1,
        'random_state': 42,
        'feature_extractors': {
            'bow': {'max_features': 500},
    ...
    pipeline = SimpleSentimentAnalysisPipeline(config)
    report = pipeline.run_pipeline()
    
    print("\nSimplified training completed! Check the following directories:")
    print("- models/: Trained models")
    print("- results/: Evaluation results and reports")
    print("- plots/: Visualizations")
    print("\nNow you can run: python -m streamlit run web_app_simple.py")
